...
DEFAULT_SWELL_ROOT = Path("/Users/claydunford/Development/Combined tool test")
...
def _ensure_import_paths(swell_root: Path, recovery_root: Path) -> None:
    for root in (swell_root, recovery_root, recovery_root / "src"):
...
def benchmark_dataset(
    ...
    *,
    ...
    from swell.host.event_detection import detector as vendored_detector
    from swell.host.event_detection.grid import build_detector_grid as vendored_build_grid
    from swell.host.event_detection.traces import detrend_traces as vendored_detrend_traces
    from swell.host.event_detection.traces import extract_lower_median_traces as vendored_extract_traces
    ...
    with timings.measure("resolve_inputs"):
        resolved = recovery_sdproj.resolve_sdproj_all_events(sdproj_path, swell_root=swell_root)
    ...
    with timings.measure("research_stack_open"):
        research_stack = recovery_proto.open_stack_source(stack_dir, swell_root)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark vendored event auto-detection against the Recovery Metric research pipeline.")
    ...
    parser.add_argument("--swell-root", default=str(DEFAULT_SWELL_ROOT), help="Combined Tool repo root.")
    ...
    parser.add_argument("--limit", type=int, default=None, help="Optional maximum number of .swell files to benchmark.")
...
def main() -> int:
    ...
    swell_root = Path(args.swell_root).expanduser().resolve()
    output_dir = Path(args.output_dir).expanduser().resolve()

    _ensure_import_paths(swell_root, recovery_root)
    sdproj_paths = sorted(p for p in data_dir.glob("*.sdproj") if p.is_file())
    if args.limit is not None:
        sdproj_paths = sdproj_paths[: max(0, int(args.limit))]
    if not sdproj_paths:
        raise FileNotFoundError(f"No .swell files found in {data_dir}")

    tmp_root = Path(tempfile.mkdtemp(prefix="event_detection_benchmark_"))
    results: list[DatasetBenchmark] = []
    details: list[dict[str, Any]] = []
    candidate_rows: list[dict[str, Any]] = []
    timing_rows: list[dict[str, Any]] = []
