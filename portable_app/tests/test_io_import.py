...
class IOImportTests(unittest.TestCase):
    def test_single_multipage_tiff_file_expands_to_multiple_frames(self):
        dummy = _DummyIO()
        with patch("app.core.io.tifffile.imread", return_value=np.zeros((3, 4, 4), dtype=np.uint16)):
    ...
    def test_mixed_file_selection_loads_supported_extensions(self):
        ...
        with patch("app.core.io.tifffile.imread", side_effect=fake_tiff_read), patch(
            "app.core.io.cv2.imread", side_effect=fake_cv_read
        ):
    ...
    def test_dimension_mismatch_frames_are_skipped(self):
        ...
        with patch("app.core.io.tifffile.imread", side_effect=fake_tiff_read):
    ...
    def test_file_selection_order_preserved(self):
        dummy = _DummyIO()
        with patch("app.core.io.cv2.imread", return_value=np.zeros((4, 4, 3), dtype=np.uint8)):
    ...
    def test_browse_input_files_mixed_valid_invalid_warns_and_keeps_valid(self):
        dummy = _DummyIO()
        with patch("app.core.io.resolve_existing_directory", return_value="."), patch(
            "app.core.io.filedialog.askopenfilenames",
            ...
            "app.core.io.messagebox.showwarning"
        ) as warn_mock:
    ...
    def test_browse_input_folder_clears_selected_files(self):
        ...
        with patch("app.core.io.resolve_existing_directory", return_value="."), patch(
            "app.core.io.filedialog.askdirectory", return_value="/tmp/images"
        ):
