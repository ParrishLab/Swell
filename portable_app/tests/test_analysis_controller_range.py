...
class AnalysisControllerRangeTests(unittest.TestCase):
    def _make_controller(self, analysis_range, nonempty_frames):
        ...
        patches = [
            patch("app.core.analysis_controller.extract_primary_boundary", return_value=np.array([[0.0, 0.0], [1.0, 1.0]])),
            patch("app.core.analysis_controller.smooth_boundary_fft", side_effect=lambda b, n_keep: b),
            patch("app.core.analysis_controller.compute_frame_metrics", side_effect=fake_compute_frame_metrics),
            patch("app.core.analysis_controller.compute_roi_metrics", side_effect=fake_compute_roi_metrics),
            patch("app.core.analysis_controller.write_metrics_outputs", side_effect=fake_write_metrics_outputs),
            patch("app.core.analysis_controller.generate_metrics_plots", return_value=None),
            patch("app.core.analysis_controller.messagebox.showinfo", return_value=None),
            patch("app.core.analysis_controller.messagebox.askyesno", return_value=True),
        ]
    ...
    def test_metrics_use_only_analysis_range_frames(self):
        ...
        with patches[0], patches[1], patches[2], patches[3], patches[4], patches[5], patches[6], patches[7], patch(
            "app.core.analysis_controller.messagebox.showwarning", return_value=None
        ):
    ...
    def test_empty_filtered_range_shows_specific_warning(self):
        ...
        with patches[0], patches[1], patches[2], patches[3], patches[4], patches[5], patches[6], patches[7], patch(
            "app.core.analysis_controller.messagebox.showwarning"
        ) as warn_mock:
            controller.run_metrics_analysis()
