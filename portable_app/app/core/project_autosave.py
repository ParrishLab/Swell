...
class ProjectAutosaveManager:
    ...
    def _slot_filename(self, tag: str, idx: int) -> str:
        if tag == "autosave":
            return f"autosave_{idx}.sdproj"
        return f"{tag}_autosave_{idx}.sdproj"
    ...
    def _all_autosave_candidates(self) -> list[Path]:
        candidates = [p for p in self.autosave_dir.glob("*.sdproj") if p.is_file()]
        out = []
        for p in candidates:
            name = p.name.lower()
            if re.fullmatch(r"autosave_\d+\.sdproj", name) or re.fullmatch(r".+_autosave_\d+\.sdproj", name):
                out.append(p)
        return out
