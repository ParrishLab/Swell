...
from app.ui.widgets import build_preview_overlay
...
class LayoutBuilder:
    def setup_ui(self):
        ...
        ttk.Label(control_frame, text="SAM2 Model:").grid(row=0, column=3, sticky="e")
        self.entry_model = tk.Entry(control_frame, width=35)
        self.entry_model.grid(row=0, column=4, padx=5)
        self.entry_model.insert(0, str(self.config.model_path()))

        self.btn_load_model = ttk.Button(
            control_frame,
            text="Browse Model...",
            command=self.on_browse_model,
        )
        self.btn_load_model.grid(row=0, column=5, padx=5)
