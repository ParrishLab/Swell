...
from app.core.interaction_controller import InteractionController
from app.core.project_autosave import AutosaveSnapshot, ProjectAutosaveManager
from app.core.project_schema import utc_now_iso
from app.core.project_store import ProjectStore, cleanup_stale_temp_files
...
from app.core.preview_resize import do_resize_preview as do_preview_resize
from app.core.preview_resize import stop_resize_preview as do_stop_resize_preview
from app.core.propagation_progress import PropagationProgressLogger
...
class SDSegmentationApp(LayoutBuilder, IOActions, SegmentationActions, RenderActions, UndoActions):
    ...
    def _apply_runtime_icon(self):
        try:
            icon_path = Path(self.app_root) / "assets" / "app_icon_runtime.png"
    ...
    def _validate_assets(self):
        ...
        configs_root = os.path.join(self.app_root, "configs")
        if not os.path.exists(configs_root):
            missing.append("configs folder")

        if missing:
            msg = "Missing " + ", ".join(missing) + ". Place assets in portable_app/models and portable_app/configs."
    ...
    def on_browse_model(self):
        ...
        if current_text:
            ...
            if not p.is_absolute():
                p = Path(self.app_root) / p
            current_abs = str(p.resolve())

        initialdir = str((Path(self.app_root) / "models").resolve())
        if current_text:
            try:
                p = Path(current_text)
                if not p.is_absolute():
