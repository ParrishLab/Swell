...
def test_popup_range_idx_to_x_uses_imported_linear_helper() -> None:
    ...
    app._mark_range_canvas = _Canvas()
...
def test_apply_popup_range_bounds_keeps_event_markers_visible() -> None:
    ...
    app._mark_scale = _Scale()
    app._mark_popup_current_idx = 150
    app._mark_processed_cache = {}
    ...
    assert app._mark_popup_local_start == 120
    assert app._mark_popup_local_end == 180
    assert app._mark_range_start_idx == 120
    assert app._mark_range_end_idx == 180
    assert app._mark_scale.configured["from_"] == 120
    assert app._mark_scale.configured["to"] == 180
