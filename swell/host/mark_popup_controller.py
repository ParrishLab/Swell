...
class MarkPopupController:
    ...
    def _begin_popup_session(self, range_start: int, range_end: int, *, normalize_to_current_frame: bool = False) -> None:
        ...
        if normalize_to_current_frame:
            current_idx = int(getattr(self.app._popup, "mark_popup_current_idx", range_start))
    ...
    def open_popup(self, mode: str, event_id: str | None) -> None:
        if self.app.reader is None or self.app.stack_info is None:
            ...
            messagebox.showwarning("Mark Event", "Load a stack first.", parent=self.app.root)
            return
        ...
        if mode == "edit" and event is None:
            ...
            messagebox.showwarning("Event", "Selected event was not found.", parent=self.app.root)
            return
        ...
        popup.title("Mark Event" if mode == "new" else f"Edit Event ({self._event_display_name(event_id)})")
    ...
    def delete_selected_events(self) -> None:
        ...
        if not ids:
            ...
            messagebox.showwarning("Event", "Select one or more events first.", parent=self.app.root)
            return
