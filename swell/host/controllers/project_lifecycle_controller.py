...
class HostProjectLifecycleController:
    ...
    def _existing_project_save_target(self, path: str | Path) -> Path | None:
        try:
            ...
            self.app._show_warning("Save Swell Project", f"Invalid project path:\n\n{path}\n\nDetails: {exc}")
            return None
        ...
        if not target.is_file():
            self.app._show_warning("Save Swell Project", f"Project save target is not a file:\n\n{target}")
            return None
        return target
    ...
    def save_project(self) -> None:
        ...
        self._task_runner().start(
            ...
            on_success=on_done,
            on_error=lambda exc: (self.app._show_warning("Save Swell Project", str(exc)), self.app._set_status("Save failed.")),
        )
    ...
    def save_project_as(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            parent=self._dialog_parent(),
            title="Save Swell Project As",
            defaultextension=PROJECT_EXTENSION,
            filetypes=[(PROJECT_FILETYPE_LABEL, "*.swell"), ("Legacy Swell Project", "*.sdproj"), ("All files", "*.*")],
    ...
    def open_project_dialog(self) -> None:
        path = filedialog.askopenfilename(
            parent=self._dialog_parent(),
            title="Open Swell Project",
            filetypes=[
                (PROJECT_FILETYPE_LABEL, "*.swell"),
                ("Legacy Swell Project", "*.sdproj"),
                ("All files", "*.*"),
            ],
        )
        if not path:
    ...
    def open_project_request(self, path: str | Path) -> bool:
        ...
        if not raw:
            self.app._show_warning("Open Swell Project", "Project path is empty.")
            return False
        try:
            candidate = Path(raw).expanduser()
        except Exception as exc:
            self.app._show_warning("Open Swell Project", f"Invalid project path:\n\n{raw}\n\nDetails: {exc}")
            return False
        if candidate.suffix.lower() not in SUPPORTED_PROJECT_EXTENSIONS:
            self.app._show_warning("Open Swell Project", "Unsupported project format. Expected .swell or legacy .sdproj")
            return False
        try:
            ...
            self.app._show_warning("Open Swell Project", f"Unable to resolve project path:\n\n{candidate}\n\nDetails: {exc}")
            return False
        if not exists or not is_file:
            self.app._show_warning("Open Swell Project", f"Project file not found:\n\n{resolved}")
            return False
    ...
    def open_project(self, path: str) -> None:
        ...
        def _on_done(payload: dict[str, object]) -> None:
            ...
            if reader is not None and stack_info is not None:
                ...
                clear_dc_runtime()
            if warning:
                self.app._show_warning("Open Swell Project", warning)
        ...
        self._task_runner().start(
            ...
            on_error=lambda exc: (
                ...
                self.app._show_warning("Open Swell Project", str(exc)),
            ),
        )
        ...
        return bool(
            messagebox.askyesno(
                ...
                (
                    ...
                    f"Saving will copy {added} into the .swell file.\n\n"
                    "Continue saving?"
                ),
                parent=self._dialog_parent(),
            )
