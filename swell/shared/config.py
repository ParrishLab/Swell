...
from swell.shared.utils.paths import get_app_root, get_resources_root, resolve_path, get_runtime_root
from swell.shared.services.checkpoint_runtime_service import is_managed_uri
...
def _runtime_config_dir() -> Path:
    ...
    if sys.platform.startswith("win"):
        ...
        if base:
            return Path(base) / "Swell"
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "Swell"
    return Path.home() / ".config" / "swell"
...
def _migrate_legacy_config_file() -> None:
    legacy_dir = _legacy_runtime_config_dir()
    if legacy_dir is None:
        return
    target_path = _runtime_config_dir() / "config.json"
