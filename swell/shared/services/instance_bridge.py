...
def _default_port() -> int:
    override = str(env_with_legacy("SWELL_INSTANCE_BRIDGE_PORT", "SDAPP_INSTANCE_BRIDGE_PORT") or "").strip()
    if override:
        try:
            value = int(override)
        except ValueError:
    ...
    digest = hashlib.sha256(f"swell.instance_bridge:{user}".encode("utf-8")).digest()
